                    ...
                    if num_eligible_sprints > 0:
                        ...
                        if allocated_effort > 0:
                            ...
                            sprint_allocations[sprint_name][project_type].append({
                                'project_effort': f"{{int(epic['projects_EA_Number']) if epic['projects_EA_Number'] and str(epic['projects_EA_Number']).isdigit() else 'No EA Number'}} ({'A' if project_type == 'anchor' else 'NA'}) - {epic_title} ({allocated_effort}) [Below Min Points]"
                            })
